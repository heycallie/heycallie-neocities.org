...
function closeTab(id) {
  // default to the first real window instead of “window”
  id = id || "window1";
...
function bringToFront(el) {
  if (!el) return; // guard against null calls
  var max = 0;
  ...
  el.style.zIndex = max + 1; // raise clicked window
}
...
function pageSource() {
  window.open("window1source.txt", "_blank");
}
